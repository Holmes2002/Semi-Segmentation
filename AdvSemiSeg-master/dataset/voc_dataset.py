...
class VOCDataSet(data.Dataset):
    def __init__(self, root, list_path, max_iters=None, crop_size=(321, 321), mean=(128, 128, 128), scale=True, mirror=True, ignore_label=255,type='sup'):
        self.root = root
        ...
        if self.type == 'sup':
            self.data_custom = open('labeled.txt','r').read().splitlines()

            self.data_LIP = open('LIPs.txt','r').read().splitlines()
            self.list_samples = self.data_custom + self.data_LIP 
            for name in self.list_samples:
                image_path = os.path.join("dataset", name)
                label_path = os.path.join("dataset", name.replace("jpg","png"))
                self.files.append({
                    "img": image_path,
                    "label": label_path,
                    "name": name
            ...
            root = f'dataset'
            self.list_samples = open('unlabeled.txt', 'r').read().splitlines()
            for name in self.list_samples:
...
class VOCDataTestSet(data.Dataset):
    def __init__(self, root, list_path, crop_size=(505, 505), mean=(128, 128, 128)):
        ...
        self.files = []
        # for split in ["train", "trainval", "val"]:
        self.list_samples = open('val.txt').read().splitlines()
        for name in self.list_samples:
            image_path = os.path.join("dataset", name)
            self.files.append({
                "img": image_path            })
